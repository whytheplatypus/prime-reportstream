...
class EmailScheduleEngine {
    ...
    fun createEmailSchedule(
        ...
        val user: String? = validateUser(request.headers["authorization"] ?: "", context.logger)
    ...
    /**
     * Retrieves the organization to send the email schedule to
     *
    ...
    private fun getOrgs(schedule: EmailSchedule): Iterable<String> {
        return (
            schedule.organizations.ifEmpty { fetchAllOrgs() }
            )
    }
     ...
     *  group
     *
     * @param requestToken the HTTPRequest object
     * @param logger logger
     *
    ...
    private fun getEmails(org: String, logger: Logger): List<String> {
        return try {
            ...
            if (ssws !== null) {
                ...
                val grpId = ((response1.jsonArray).get(0) as JSONObject).getString("id")
                ...
                response.jsonArray.map {
                    (it as JSONObject).getJSONObject("profile").getString("email")
                }
            } else {
                emptyList()
            }
    ...
    private fun dispatchToSendGrid(
        ...
        p.subject = SUBJECT_EMAIL

        val mail = Mail().apply {
            this.addPersonalization(p)
            this.subject = SUBJECT_EMAIL
            this.from = Email(FROM_EMAIL)
            this.templateId = template
        }

        // fetch the secret or log failure to fetch
        SecretHelper.getSecretService().fetchSecret("SENDGRID_ID")?.let { sendgridId ->
            val sg = SendGrid(sendgridId)
            val request = Request()
            try {
